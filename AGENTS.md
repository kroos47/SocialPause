...
- Each stopped-to-running Start refreshes usage while respecting the current lunch window and locks the main Stop action for six continuous elapsed hours. Repeated Start while running must do nothing. Enforce this in the engine/controller, not just the disabled button. Home shows “Starting locks Stop for 6 hours.” while stopped and “Stop available in HH:MM:SS” while locked; at the exact deadline Stop enables without stopping monitoring. Lunch, cooldowns, sleep, and screen-off time count toward the lock. Lunch controls neither reset nor extend it. Once enabled, Stop grants unrestricted access and hides notifications.
- Ordinary process recreation, reopening, Recents removal and app updates preserve every app deadline/allowance and the Stop lock, while clearing stale focus. Reboot or confirmed Force stop leaves monitoring stopped until Start, with no timer notifications/alarms; reboot may reset ordinary usage. Use current-process startup evidence on Android 15+ for Force stop, never an old startup record or a generic process restart. When reliable evidence is unavailable, preserve state. Lunch phases, history and daily records survive. Upgrade to 0.7 preserves 0.3–0.6 timers, explicitly initializes new fields, and never applies a retrospective Stop lock to an already-running installation. The 0.3 upgrade resets old timers exactly once while preserving history, schedules, selection, running status, and current lunch restrictions.
- One ongoing notification shows the focused app, or an overview of every app when none is focused. Only active app usage requests a live status chip with explicit compact MM:SS short critical text and the drawer chronometer. In shared mode use the smaller app/shared allowance, with the SocialPause icon when shared time limits use; otherwise use the app icon; idle/cooldown uses the SocialPause icon without chip text. Use a standard template for live eligibility and custom per-app rows only for the expanded idle overview. Restore dismissed ordinary notifications while monitoring is active, except Sleep Time. Dismissing ordinary lunch/cooldown/idle notifications must not suppress future live promotion. Dismissing an active live countdown suppresses promotion until the next stopped-to-running Start. Dismissal callbacks must retain their original surface and monitoring-run identity and reject stale events. Preserve ambiguous legacy dismissal records until the next Start. Never promise an unremovable Android notification.
...
- Confirmed revocation of this app’s Accessibility service stops and persists monitoring even during the six-hour Stop lock, clears overlay/notification/focus, and cancels alarms. Keep this system-stop path separate from guarded user Stop. Re-enabling requires Start. A temporary disconnect while permission stays enabled preserves the run; do not treat connected=false as revocation.
...
- For timing changes, add meaningful fake-clock scenarios for independent deadlines, delayed callbacks, lunch, process recovery, and migration. Cover the six-hour exact boundary, clock changes, rejected Stop/repeated Start, same-boot lock persistence, reboot/confirmed Force stop, and old active runs without retrospective locks. Keep the legacy serialized fixtures and randomized reference-model coverage.
- Verify notification dismissal/restoration, focused/overview transitions, day selection, dark mode, and large text at runtime; separate emulator evidence from Samsung acceptance. The opt-in `RuntimeChecks` Android instrumentation uses synthetic data and must run only on a disposable emulator, never a personal phone. Build its separate APK with `:app:assembleDebugAndroidTest`; see docs/ANDROID_STUDIO.md.
...
- Keep the Gradle wrapper JAR and synthetic `legacy-v02.bin` / `legacy-v04-*.bin` / `legacy-v05-*.bin` / `legacy-v06-*.bin` test fixtures. Generated `engine/bin` class files are local output, not source.
