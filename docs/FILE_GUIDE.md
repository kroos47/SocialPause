...
| `MainActivity.java` | Home, Insights and Settings navigation, Start/Stop, app picker, schedule dialogs, permission setup, and system-bar insets. Native Views are constructed here rather than in layout XML. |
...
| `TimerNotifications.java` | Silent ongoing notification, remaining allowances, countdown chronometer, sleep hiding, and best-effort promoted-notification request. |
...
| File | Responsibility |
|---|---|
| `app/src/main/java/app/socialpause/Design.java` | Shared light/dark colors, cards, icons, ring, progress bars and chart drawing. |
...
| `app/src/main/java/app/socialpause/NotificationDismissReceiver.java` | Remembers a user-dismissed notification for the current allowance cycle, without changing enforcement. |
...
| `engine/src/main/java/app/socialpause/engine/TimerPresentation.java` | Testable notification state: app/shared limiting allowance, paused, lunch, cooldown, or hidden. |
...
| File | Responsibility |
|---|---|
| `engine/src/main/java/app/socialpause/engine/RulesEngine.java` | Platform-independent state machine: per-app/shared budgets, pauses, lunch, cooldown, sleep window, pending schedule edits, and restore behavior. |
| `engine/src/test/java/app/socialpause/engine/EngineTests.java` | 25 scenario tests, including 5,000 randomized focus/lock transitions. Uses a fake clock and no JUnit dependency. |
...
Android Accessibility event
  → SocialAccessibilityService identifies the focused package and lock state
