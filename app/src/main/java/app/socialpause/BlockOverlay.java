...
final class BlockOverlay {
    ...
    void show(String pkg,RulesEngine e) {
        ...
        engine=e;blockedPackage=pkg;Design d=new Design(service);
        ...
        TextView title=d.text(lunch?"Time for a breather":AppController.label(service,pkg)+"’s "+e.limit(pkg)/RulesEngine.MINUTE+"-minute limit reached",26,d.ink,true);title.setGravity(Gravity.CENTER);d.add(card,title,24);
        long left=e.cooldownRemaining(pkg,AppController.wall(),AppController.elapsed());
        TextView message=d.text(lunch?"Your lunch break is complete. All selected apps are resting.":"A little space for now. Other apps keep their own timers.",14,d.muted,false);message.setGravity(Gravity.CENTER);d.add(card,message,16);
        remaining=d.text(AppController.duration(left),40,d.ink,true);remaining.setGravity(Gravity.CENTER);d.add(card,remaining,24);
        TextView unlock=d.text("Available again at "+AppController.at(AppController.wall()+left),13,d.muted,false);unlock.setGravity(Gravity.CENTER);d.add(card,unlock,8);
    ...
    private final Runnable tick=new Runnable(){@Override public void run(){if(view==null)return;long left=dismissAt-SystemClock.elapsedRealtime();if(left<=0){goHome();return;}remaining.setText(AppController.duration(engine.cooldownRemaining(blockedPackage,AppController.wall(),AppController.elapsed())));countdown.setText(service.getString(R.string.returning_home,(left+999)/1000));handler.postDelayed(this,250);}};
